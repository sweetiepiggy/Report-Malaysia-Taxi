...
public class ReportMalaysiaTaxiActivity extends Activity {
	...
	private void restore_saved_state(Bundle savedInstanceState) {
		...
		mData.submit_selected = savedInstanceState
		...
		mData.youtube_checked = savedInstanceState
				.getBoolean("youtube_checked");

		mData.photoUris = strarr2uriarr(savedInstanceState
				.getStringArrayList("photo_uris"));
		mData.recordingUris = strarr2uriarr(savedInstanceState
				.getStringArrayList("recording_uris"));
		mData.videoUris = strarr2uriarr(savedInstanceState
				.getStringArrayList("video_uris"));
		mData.attachmentUris = strarr2uriarr(savedInstanceState
	...
	private void init_camera_recorder_buttons() {
		...
		if (has_microphone) {
			recorder_button.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					Intent recorder_intent = new Intent(
	...
	private void init_entries(DataWrapper data) {
		...
		String photo_size = data.photoUris.size() > 0 ? Integer
				.toString(data.photoUris.size()) : "";
		String video_size = data.videoUris.size() > 0 ? Integer
				.toString(data.videoUris.size()) : "";
		String recording_size = data.recordingUris.size() > 0 ? Integer
				.toString(data.recordingUris.size()) : "";
		String attachment_size = data.attachmentUris.size() > 0 ? Integer
		...
		((TextView) findViewById(R.id.attachment_label))
	...
	private void init_submit_button() {
		...
		submit_button.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (((EditText) findViewById(R.id.reg_entry)).getText()
						.toString().length() == 0) {
					results_complete = false;
					incomplete_msg = getResources().getString(
							R.string.missing_reg);

					/* TODO: don't hardcode "Other" */
				} else if ((mData.email_offence.equals("Other") || mData.email_offence
						.equals("Lain-lain"))
						&& ((EditText) findViewById(R.id.details_entry))
								.getText().toString().length() == 0) {
					results_complete = false;
					incomplete_msg = getResources().getString(
							R.string.missing_details);
				}
	...
	private void submit_menu() {
		...
		builder.setMultiChoiceItems(submit_choices, mData.submit_selected,
	...
	private void submit() {
		String date = String.format("%02d/%02d/%04d", mData.day,
				mData.month + 1, mData.year);
		String time = format_time(mData.hour, mData.minute);
		String loc = ((EditText) findViewById(R.id.location_entry)).getText()
				.toString();
		String reg = ((EditText) findViewById(R.id.reg_entry)).getText()
				.toString();
		String details = ((EditText) findViewById(R.id.details_entry))
				.getText().toString();
		...
		String msg = format_msg(date, time, loc, reg, mData.email_offence,
				details);
	...
	private void send_youtube(String msg) {
		String action = mData.videoUris.size() > 1 ? Intent.ACTION_SEND_MULTIPLE
				: Intent.ACTION_SEND;
		...
		startActivityForResult(
				Intent.createChooser(youtube_intent,
	...
	private void send_email(final String msg, final String reg) {
		...
		builder.setMultiChoiceItems(R.array.email_choices, mData.who_selected,
				new DialogInterface.OnMultiChoiceClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which,
							boolean is_checked) {
	...
	private void send_tweet(String date, String time, String loc, String reg,
			String details) {
		String tweet_msg = format_tweet(date, time, loc, reg, mData.offence,
				details);
		...
		startActivityForResult(
				Intent.createChooser(tweet_intent,
	...
	private String format_msg(String date, String time, String location,
		...
		if (reg.length() != 0) {
			message += '\n' + getResources().getString(R.string.email_reg)
					+ ": " + reg;
		}
		if (location.length() != 0) {
			message += '\n' + getResources().getString(R.string.email_loc)
					+ ": " + location;
		}
		/* TODO: don't hardcode "Other" */
		if (offence.length() != 0 && !offence.equals("Other")) {
			message += '\n' + getResources().getString(R.string.email_offence)
					+ ": " + offence;
		}
		if (details.length() != 0) {
			if (offence.length() != 0 && !offence.equals("Other")) {
	...
	private String format_tweet(String date, String time, String loc,
		...
		if (details.length() == 0
				|| build_tweet(map, date, time, loc, reg, offence, details)
						.length() > max_length) {
		...
		if (build_tweet(map, date, time, loc, reg, offence, details).length() > max_length) {
			map.put("twitter_address2", false);
		}
	...
	private void update_date_label(int year, int month, int day) {
		...
		String date = translate_day_of_week(DateFormat.format("EEEE", d)
				.toString())
				+ " "
				+ DateFormat.getLongDateFormat(getApplicationContext()).format(
						d);
	...
	private void update_time_label(int hour, int minute) {
		Button time_button = (Button) findViewById(R.id.time_button);
		String time = DateFormat.getTimeFormat(getApplicationContext()).format(
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case ACTIVITY_TAKE_PHOTO:
			if (resultCode == RESULT_OK) {
				...
				((TextView) findViewById(R.id.camera_label)).setText(Integer
						.toString(mData.photoUris.size()));
			}
			break;
		case ACTIVITY_TAKE_VIDEO:
			if (resultCode == RESULT_OK) {
				mData.videoUris.add(data.getData());
				((TextView) findViewById(R.id.video_label)).setText(Integer
						.toString(mData.videoUris.size()));
		...
		case ACTIVITY_RECORD_SOUND:
			if (resultCode == RESULT_OK) {
				...
				((TextView) findViewById(R.id.recorder_label)).setText(Integer
						.toString(mData.recordingUris.size()));
			}
			break;
		case ACTIVITY_GET_ATTACH:
			if (resultCode == RESULT_OK) {
				...
				((TextView) findViewById(R.id.attachment_label))
						.setText(Integer.toString(mData.attachmentUris.size()));
			}
			break;
	...
	public class OffenceOnItemSelectedListener implements
		...
		public void onItemSelected(AdapterView<?> parent, View view, int pos,
				long id) {
			mData.offence = getResources()
					.getStringArray(R.array.offence_array)[pos];
			mData.email_offence = getResources().getStringArray(
					R.array.email_offence_array)[pos];
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.fare_rate:
			Intent fare_rate_intent = new Intent(getApplicationContext(),
					FareRateActivity.class);
			startActivity(fare_rate_intent);
			return true;
		case R.id.fare_calc:
			Intent fare_calc_intent = new Intent(getApplicationContext(),
					FareCalcActivity.class);
			startActivity(fare_calc_intent);
			return true;
		case R.id.contacts:
			Intent contacts_intent = new Intent(getApplicationContext(),
					ContactsActivity.class);
			startActivity(contacts_intent);
			return true;
		case R.id.about:
			Intent about_intent = new Intent(getApplicationContext(),
					TextViewActivity.class);
			Bundle b = new Bundle();
