...
public class ReportMalaysiaTaxiActivity extends Activity
{
	...
	private void init_entries(DataWrapper data)
	{
		...
		String video_size = data.videoUris.size() > 0 ? Integer.toString(data.videoUris.size())
		        : "";
		String recording_size = data.recordingUris.size() > 0 ? Integer.toString(data.recordingUris
		        .size()) : "";
			...
			public void onClick(View v) {
				...
				if (((EditText) findViewById(R.id.reg_entry)).getText().toString().length() == 0) {
					...
					/* TODO: don't hardcode "Other" */
				        ...
				        .equals("Lain-lain"))
				        && ((EditText) findViewById(R.id.details_entry)).getText().toString()
				                .length() == 0) {
					results_complete = false;
	...
	private void init_selected(DataWrapper data)
	{
		...
		data.who_selected = new boolean[] {
		        true, true, true, false, false, false
		};
		data.submit_selected = new boolean[] {
	        ...
	        String reg, String offence, String details)
	{
		...
		if (build_tweet(map, date, time, loc, reg,
		        offence, details).length() > max_length) {
			map.put("twitter_address2", false);
		}
		...
		switch (requestCode) {
			...
			case ACTIVITY_GET_ATTACH:
				if (resultCode == RESULT_OK) {
					...
					((TextView) findViewById(R.id.attachment_label)).setText(Integer
					        .toString(mData.attachmentUris.size()));
				}
				break;
			case ACTIVITY_SUBMIT:
				/*
				 * repeatedly submit until all send_*() functions have been
				 * called
				 */
				submit();
				break;
